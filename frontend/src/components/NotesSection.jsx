...
const NotesSection = () => {
  const { categories, subjects, notes, fetchCategories, fetchSubjects, fetchNotes } = useNotes();
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="space-y-6">
          {categories.map((category) => (
            <div key={category.name} className="bg-white rounded-xl shadow-sm border border-gray-100">
              <button
                className="w-full flex items-center justify-between p-4 hover:bg-gray-50 transition-colors"
                onClick={() => setExpandedCategory(expandedCategory === category.name ? null : category.name)}
              >
                <span className="text-lg font-semibold text-gray-800">{category.name}</span>
                <ChevronRight className={`transform transition-transform duration-200 ${expandedCategory === category.name ? "rotate-90" : ""}`} />
              </button>
                  ...
                  {subjects.map((subject) => (
                    <div key={subject.name} className="border-b border-gray-100 last:border-b-0">
                      ...
                      >
                        ...
                        <ChevronRight className={`transform transition-transform duration-200 ${expandedSubject === subject.name ? "rotate-90" : ""}`} />
                      </button>

                      {expandedSubject === subject.name && (
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 p-4 bg-gray-50">
                          {notes.map((note) => (
                            <div key={note.id} className="bg-white rounded-lg shadow-sm overflow-hidden hover:shadow-md transition-shadow">
                              <img src={note.imgUrl} alt={note.title} className="w-full h-40 object-cover" />
                              <div className="p-4">
